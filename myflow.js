(function ($) {
    ...
    myflow.path = function (o, r, from, to, guid, ec,dots,id) {
        ...
        // 转化json数据
        this.toJson = function () {
            var data = "{lineID:'" + (!_o.lineID ? "" : _o.lineID) + "',lineType:'path',from:'" + _from.getId() + "',to:'" + _to.getId()
    ...
    myflow.jpath = function (o, r, from, to, guid, ec,dots,id) {
        var _this = this, _r = r, _o = $.extend(true, {}, myflow.config.jpath), _path,_markpath, _arrow, _text, _textPos = _o.text.textPos, _ox, _oy, _from = from, _to = to, _id = id || 'path'
            + myflow.util.nextId(), _dotList, _autoText = true; _o.lineID = guid; oec = (ec > 0 ? (parseInt(ec) == 1 ? 25 : parseInt(ec) * 9 + 22) : 0);
        ...
        // 转化json数据
        this.toJson = function () {
            var data = "{lineID:'" + (!_o.lineID ? "" : _o.lineID) + "',lineType:'jpath',from:'" + _from.getId() + "',to:'" + _to.getId()
                + "', dots:" + _dotList.toJson() + ",text:{text:'"
                + _text.attr('text') + "',textPos:{x:"
                + Math.round(_textPos.x) + ",y:" + Math.round(_textPos.y)
                + "}}, props:{";
    ...
    myflow.lpath = function (o, r, from, to, guid, ec,dots,id) {
        var _this = this, _r = r, _o = $.extend(true, {}, myflow.config.lpath), _path,_markpath, _arrow, _text, _textPos = _o.text.textPos, _ox, _oy, _from = from, _to = to, _id = id || 'path'
            + myflow.util.nextId(), _dotList, _autoText = true; _o.lineID = guid; oec = (ec > 0 ? (parseInt(ec) == 1 ? 25 : parseInt(ec) * 9 + 22) : 0);
        ...
        // 转化json数据
        this.toJson = function () {
            var data = "{lineID:'" + (!_o.lineID ? "" : _o.lineID) + "',lineType:'lpath',from:'" + _from.getId() + "',to:'" + _to.getId()
                + "', dots:" + _dotList.toJson() + ",text:{text:'"
                + _text.attr('text') + "',textPos:{x:"
                + Math.round(_textPos.x) + ",y:" + Math.round(_textPos.y)
                + "}}, props:{";
    ...
    myflow.cpath = function (o, r, from, to, guid, ec,dots,id) {
        var _this = this, _r = r, _o = myflow.config.cpath, _path,_markpath, _arrow, _text, _textPos = _o.text.textPos, _ox, _oy, _from = from, _to = to, _id = id || 'path'
            + myflow.util.nextId(), _dotList, _autoText = true; _o.lineID = guid; oec = (ec > 0 ? (parseInt(ec) == 1 ? 25 : parseInt(ec) * 9 + 22) : 0);
        //var data=from.toJson();

        var arg0={
            key:"from",
            eqType:from.getEquipmentType(),
            toEqType:to.getEquipmentType()
        };
        var arg1={
            key:"to",
            eqType:to.getEquipmentType()
        };
        if(from.getType()!="task"){
        ...
        //这里写前端校验，先看to，根据to的类型，去取from的selectEditor
        //因为to是输出，只有符合要求的输入才能进来。
        //所以传两个参数么？
        
        $.extend(_o.props,{
            temp1: {name:'temp1', label : '文本', value:'', editor: function(){return new myflow.editors.inputEditor();}}
        });
        _o.props.tmp0["editor"]=function() {
                        return new myflow.editors.selectEditor(arg0);
                    };
        
        _o.props.tmp1["editor"]=function() {
                        return new myflow.editors.selectEditor(arg1)
                    };
        ...
        //修改设备类型会删除连这的参数线
        var removecpathHandler = function (e, v ,src) {
            if (!myflow.config.editable)
                return;
            debugger;
            console.log(v);
        ...
        $(_r).bind('removerect', removerectHandler);
        $(_r).bind('equipmentTypeChange',removecpathHandler)
        ...
        // 转化json数据
        this.toJson = function () {
            var data = "{lineID:'" + (!_o.lineID ? "" : _o.lineID) + "',lineType:'cpath',from:'" + _from.getId() + "',to:'" + _to.getId()
                + "', dots:" + _dotList.toJson() + ",text:{text:'"
                + _text.attr('text') + "',textPos:{x:"
                + Math.round(_textPos.x) + ",y:" + Math.round(_textPos.y)
                + "}}, props:{";
            ...
            function saveCheck(){                
                ...
                for(var i in _states){
                    if(_states[i].getType()=="start"||_states[i].getType()=="end"){ 
                        flag+=1;
                    }else if(_states[i].getType()=="task"){
